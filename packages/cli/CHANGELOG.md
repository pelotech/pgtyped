...
- result types for Postgres array columns now declare their elements nullable. A `text[]` result is `nullableStringArray = (string | null)[]` where it was `stringArray = (string)[]`, and code that indexes or maps an array result without a null check stops compiling. Parameter types are unchanged: an array parameter is still `(T)[]` under its old name. Relatedly, a `typesOverrides` entry naming only one scope no longer leaks into the other for an array element type.
...
- type the elements of a Postgres array result as nullable ([#30](https://github.com/pelotech/pgtyped/issues/30)) ([197c625](https://github.com/pelotech/pgtyped/commit/197c625))
...
- never let pgliteTypeDb describe return an empty description ([#43](https://github.com/pelotech/pgtyped/issues/43)) ([268e038](https://github.com/pelotech/pgtyped/commit/268e03842176abba8e09711221940f55e3efa603))
