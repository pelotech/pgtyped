...
two columns of another, and runs codegen as that role. The same test then runs the same project as
`postgres` and asserts that the run warns it is a superuser: a superuser bypasses every privilege
check, so a clean run as one reports nothing, and the check now says so — once per run, naming the
role, fatal under `failOnError`. The role is read from `pg_roles` at `current_user`, which follows
`SET ROLE` (measured against 18.6 and against PGlite; `current_setting('is_superuser')` follows it
too, but the catalog row is the documented meaning and carries the name in the same round trip).
...
| **#561** permission reflection               | Not implemented                                                                                                                                                                                                                                    | `db/describe.ts` derives types from Parse/Describe, which carries no grant information, so the limitation the reporter deduced still holds for _typing_. Grants are no longer entirely invisible, though: the opt-in `checkPrivileges` added for PR #563 plans each query and reports what the role may not execute — and warns, once per run, when that role is a superuser, since a superuser passes every query and the check would otherwise report nothing.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **#513** DDL instead of a live database      | The CLI itself still requires a live connection                                                                                                                                                                                                    | **The blocker is gone.** `main` takes an optional `db`, and `pgliteTypeDb` (behind `@pelotech/pgtyped-cli/pglite`) backs it with an in-process PGlite, so a script can apply its migrations and generate types with no server anywhere — see [PR #620](#pr-620--a-programmatic-api-minus-the-package-split) and "Generating without a server" in `docs-new/docs/cli.md`. What is deliberately **not** built is the reporter's literal ask: nothing reads a `.sql` file as a schema. "Apply my DDL" is really "apply my migrations in order", which is a config surface this package has no opinion about, so it stays the caller's script. Two real limits remain: a default PGlite carries only `plpgsql` and the 34 bundled contrib extensions must be named in the _constructor_ (a bare `CREATE EXTENSION` fails `0A000`), and there is no PostGIS, pgvector or TimescaleDB at all. |
|                                              |
...
anything unknown: its PGlite premise was the open question and has been confirmed by building it.
What remains is a decision about when to commit to a public interface, not a technical risk.
...
move the argv/`process.exit` code out of `index.ts` into a `cli.ts` bin, and let a caller supply the
database. **All three are done.** `index.ts` is a library that exports `main`, `cli.ts` is the bin,
the package has a real `"."` entry, and `main` now takes an optional fourth argument, a `TypeDb`.
When one is supplied no `pg.Pool` is constructed, none is closed, and `verifyConnection` is skipped —
which is right rather than merely convenient: that pre-flight exists to catch an unreachable
_server_, and an injected database has none.

**The PGlite premise was the open question, and it holds.** `@electric-sql/pglite` 0.5.8 can back
`TypeDb` in full, so no part of the extended-query path had to be reimplemented, and
`pgliteTypeDb` ships behind the `./pglite` subpath export. Applying `packages/example/sql/schema.sql`
to an in-process PGlite and regenerating the example produced all eight committed files
**byte-for-byte identical** to the ones generated against `postgres:18-alpine` — enums, the domain
...
- **`db.describeQuery()` is unusable.** It returns only `{dataTypeID, serializer}` per column — no
  ...
  column comes out nullable. `execProtocol` with the exported `protocol.serialize` — `parse` +
  `describe` + `sync` concatenated — returns a full `parameterDescription` and `rowDescription`. No
  private API is touched.
- **`execProtocol` is not covered by PGlite's query lock.** A same-tick burst of nine unlocked
  describes leaves **8 of 9 empty**, reproducibly: one caller receives every backend message and the
  rest receive none, yielding an empty `Described` and therefore `never` types, with no error.
  Wrapping each call in `db.runExclusive` fixes it. `MAX_CONCURRENCY` in `util.ts` buys nothing here
  — correctness is fine, throughput is serial.
...
string — and its errors carry pg-shaped `code`, `hint` and `position`, which is what `toParseError`
and `toPrivilegeError` read.

Two further measurements worth keeping. Real `postgres:14-alpine` (14.24), `15-alpine` (15.19) and
`18-alpine` (18.6) servers each produced the same bytes as PGlite and as each other for the
example's schema, so the version skew this approach might have suffered from did not appear —
one schema measured, not a guarantee. And codegen through PGlite is not slower despite the
serialization: 172–186 ms against 167–180 ms through a five-connection pool.

`checkPrivileges` still works on an injected PGlite, but only if the caller asks for it: there is
no listener and no auth, so codegen runs as superuser unless the script issues `SET ROLE`, which
gives identical privilege semantics (measured — a revoked table yields `42501` from `explain`). The
run warns that it is a superuser until the script does, the same warning a server connection as
`postgres` earns.

**What was not taken.** `@electric-sql/pglite` is an **optional peer dependency**, not a dependency:
it is a multi-megabyte WASM blob and nobody generating against a real server should pay for it. That
is the same shape `typescript` already has here, and the adapter sits behind its own subpath export
so importing `@pelotech/pgtyped-cli` never loads it. A stable `generateTypes(file, config, db)`
surface is still not committed to — `main` takes a whole `ParsedConfig` and still ends the process on
an unreachable server when it builds the pool itself — so `main` is honest enough to import but is
not yet the public API this PR asked for. That remains a decision about what to commit to rather than
a technical risk.
...
- **#561 (permissions)**: verified only that the feature does not exist and that the code path the
  reporter describes is unchanged. No attempt was made to build it. **#513 (DDL source)** is no
  longer on this list: the injection seam and the PGlite adapter were built and the example's eight
  committed files regenerate byte-identically through them.
...
- **PR #620**: nothing outstanding. The packaging defect was reproduced and fixed, and the PGlite
  ...
  through an injected PGlite with no TCP listener. What is left is a decision about a public API
  surface, not an unknown.
...
#513 **FIXED here** (seam + adapter) · #517 **FIXED here** · #522 **FIXED here** · #523 **answered** (JS config) · #526 **FIXED here** ·
...
#614 **FIXED here** (rescoped) · #615 NA · #616 **FIXED here** · #619 NA · #620 packaging and injection seam **FIXED here**, split NA ·
