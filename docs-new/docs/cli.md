...
| `failOnError?`          | `boolean`                | Whether to fail on a file processing error and abort generation. Also promotes codegen warnings, such as a `sql.prepared` name that does not match the variable holding it, into failures. **Default:** `false`                                                                                      |
