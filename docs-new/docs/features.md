...
- **Prepared statements** - Queries written in `.sql` files are sent as server-side prepared statements by default, so Postgres parses and plans each one once per connection. Queries written inline opt in with `sql.named`.
