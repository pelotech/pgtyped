...
The schema still has to get into that database first. A tool that can be handed the instance does it directly; a tool that only takes a connection string — most migration tools, graphile-migrate among them — reaches the same instance through PGlite's socket server, then pgtyped is handed the instance as above. Both flows, the limits, and the extension story are in [Generating without a server](./docs-new/docs/cli.md#generating-without-a-server).
